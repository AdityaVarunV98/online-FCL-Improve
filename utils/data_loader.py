...
def get_loader_all_clients(args, run):
    ...
    if not os.path.exists(loader_fn):
        ...
        for client_id, cls_assignment in enumerate(cls_assignment_list):
            print(f"Client {client_id} class order:", cls_assignment)

        for client_id, client_ds in enumerate(all_clients_ds):
            print(f"\nClient {client_id}:")
            print("Train")
            for task_id, (x, y) in enumerate(client_ds[0]):  # train split
                unique, counts = np.unique(y.numpy(), return_counts=True)
                print(f"  Task {task_id}: {dict(zip(unique, counts))}")
            print("Val")
            for task_id, (x, y) in enumerate(client_ds[1]):  # val split
                unique, counts = np.unique(y.numpy(), return_counts=True)
                print(f"  Task {task_id}: {dict(zip(unique, counts))}")
            print("Test")
            for task_id, (x, y) in enumerate(client_ds[2]):  # test split
                unique, counts = np.unique(y.numpy(), return_counts=True)
                print(f"  Task {task_id}: {dict(zip(unique, counts))}")
